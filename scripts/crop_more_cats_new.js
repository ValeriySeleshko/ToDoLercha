...
async function processNewCats() {
  const srcImage = path.join(__dirname, '..', 'Sticker_Cats_New.jpg');
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const halfW = Math.floor(cellW * 0.54);
      const halfH = Math.floor(cellH * 0.54);
      ...
      // Flood-fill transparency from outer borders
      ...
      // Smooth edge matte anti-aliasing
  ...
  console.log('Successfully extracted and optimized ' + (id - 1) + ' new more_cats stickers from Sticker_Cats_New.jpg!');
}

processNewCats().catch(console.error);
